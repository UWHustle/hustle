...
Aggregate::get_group_filter(std::vector<int> its) {
  ...
  for (int i = 0; i < group_by_refs_.size(); i++) {
    ...
    for (auto &order_by_ref : order_by_refs_) {
      if (order_by_ref.table == group_by_refs_[i].table &&
        ...
        status = arrow::compute::SortToIndices(&function_context, *values,
                                               &sorted_indices);
        evaluate_status(status, __FUNCTION__, __LINE__);
        // Sort group
        status = arrow::compute::Take(&function_context, *values,
                                      *sorted_indices, take_options, &values);
        evaluate_status(status, __FUNCTION__, __LINE__);
        // Sort aggregates. Note that this may be sorted multiple times if there are multiple orderbys.
        status = arrow::compute::Take(&function_context, *aggregates,
                                      *sorted_indices, take_options,
                                      &aggregates);
        evaluate_status(status, __FUNCTION__, __LINE__);
      }
...
void Aggregate::compute_aggregates(Task *ctx) {
  ...
  std::vector<std::vector<int>> all_its;
  ...
  while (!exit) {
    ...
    // LOOP BODY START
    all_its.push_back(group_its);
    ctx->spawnLambdaTask(
        [this, group_id, group_its, aggregate_col] {
          auto group_filter = get_group_filter(group_its);
          ...
          insert_group(group_its, group_id);
        });
    // LOOP BODY END
    ...
    group_its[n - 1]++;
    while (group_its[index] == maxes[index]) {
