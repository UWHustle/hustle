...
std::vector<std::shared_ptr<arrow::ArrayBuilder>> Aggregate::get_group_builders() {
...
std::shared_ptr<arrow::ChunkedArray> Aggregate::get_group_filter(std::vector<int> group_id) {
      ...
      case arrow::Type::STRING: {
        ...
        value = arrow::compute::Datum(
            std::make_shared<arrow::StringScalar>(
                one_unique_values_casted->GetString(group_id[field_i])));
        next_filter = get_unique_value_filter(group_by_refs_[field_i], value);
        break;
      }
      case arrow::Type::INT64: {
        auto one_unique_values_casted =
  ...
  for (int i = 0; i < groupt_type_->num_children(); i++) {
    switch (groupt_type_->child(i)->type()->id()) {
      ...
      case arrow::Type::STRING: {
        ...
        status = col_group_builder->Append(col_unique_values->GetString(group_id[i]));
        evaluate_status(status, __FUNCTION__, __LINE__);
        break;
      }
      case arrow::Type::INT64: {
        ...
        // Append one of the unique values to the column's builder.
        status = col_group_builder->Append(col_unique_values->Value(group_id[i]));
        evaluate_status(status, __FUNCTION__, __LINE__);
        break;
      }
      default: {
        std::cerr << "Cannot insert unsupported aggregate type: "
                     + groupt_type_->child(i)->type()->ToString() << std::endl;
      }
    }
  }
    ...
    for (auto &order_by_ref : order_by_refs_) {
      if (order_by_ref.table == group_by_refs_[i].table &&
        ...
        status = arrow::compute::Take(
            &function_context, *values, *sorted_indices, take_options, &values);
        evaluate_status(status, __FUNCTION__, __LINE__);

        // Sort aggregates. Note that this may be sorted multiple times if
        // there are multiple orderbys.
        status = arrow::compute::Take(
            &function_context, *aggregates, *sorted_indices, take_options, &aggregates);
        evaluate_status(status, __FUNCTION__, __LINE__);
      }
    }
...
arrow::compute::Datum Aggregate::compute_aggregate(
  ...
  if (group_filter != nullptr) {
    status = arrow::compute::Filter(
        &function_context, aggregate_col, group_filter, filter_options, &datum_col);
    evaluate_status(status, __FUNCTION__, __LINE__);
...
void Aggregate::compute_aggregates(Task *ctx) {
  ...
  while (!exit) {
    ...
    if (n == 0) break; // Only execute the loop once if there are no group bys
