...
// TODO(nicholas): This entire function can be ignored.
        std::shared_ptr<Block> Join::runOperator(
            ...
            for (int i = 0; i < left->get_num_rows(); i++) {
                ...
                //  get_field() in Block.
                for (int col_index = 0;
                     col_index < right->get_num_rows(); col_index++) {
                    if (right->get_records()->schema()->field(
                            col_index)->name() !=
                        column_name_) {
...
//        out_block.insert_records(out_record_data);
        ...
        std::vector<std::shared_ptr<Block>> Join::runOperator(
            ...
            for (int i = 0; i < left_blocks.size(); i++) {
                ...
                auto schema_fields = std::vector<std::shared_ptr<arrow::Field>>();
                for (int j = 0; j <
                                left_blocks[i]->get_records()->schema()->num_fields(); j++) {
                    schema_fields.push_back(
                            left_blocks[i]->get_records()->schema()->field(
                                    j)->Copy());
                }
                for (int j = 0; j <
                                right_blocks[i]->get_records()->schema()->num_fields(); j++) {
                    if (right_blocks[i]->get_records()->schema()->field(
                            j)->name() != column_name_) {
                        schema_fields.push_back(
                                right_blocks[i]->get_records()->schema()->field(
                                        j)->Copy());
                    }
                }
                auto out_schema = arrow::schema(schema_fields);
                ...
                for (int ii = 0; ii <
                                 left_blocks[i]->get_records()->num_columns() +
                                 right_blocks[i]->get_records()->num_columns();
                     ii++) {
                ...
                // outer loop
                for (int i_o = 0; i_o <
                                  left_blocks[i]->get_records()->GetColumnByName(
                                          column_name_)->length(); i_o++) {
                    // extract outer loop value
                    arrow::compute::Datum join_val;
                    auto join_val_field = left_blocks[i]->get_records()->schema()->GetFieldByName(
                            column_name_);
                    switch (join_val_field->type()->id()) {
                        case arrow::Type::INT64: {
                            auto casted_col = std::static_pointer_cast<arrow::Int64Array>(
                    ...
                    status = arrow::compute::Compare(
                            &function_context,
                            join_val,
                            right_blocks[i]->get_records()->GetColumnByName(
                                    column_name_),
                            compare_options,
                            left_filter
                    );
                    ...
                    for (int i_i = 0; i_i <
                                      left_blocks[i]->get_records()->num_columns(); i_i++) {
                        auto *out_data = new arrow::compute::Datum();
                        status =
                                arrow::compute::Filter(&function_context,
                                                       left_blocks[i]->get_records()->column(
                                                               i_i),
                                                       *left_filter,
                                                       out_data);
                        if (!status.ok()) {
                        ...
                        out_record_data[i_i].push_back(
                                std::make_shared<arrow::compute::Datum>(
                                        out_data));
                    }
                    for (int i_i = 0; i_i <
                                      right_blocks[i]->get_records()->num_columns(); i_i++) {
                        if (right_blocks[i]->get_records()->schema()->field(
                                i_i)->name() != column_name_) {
                            auto *out_data = new arrow::compute::Datum();
                            status =
                                    arrow::compute::Filter(&function_context,
                                                           left_blocks[i]->get_records()->column(
                ...
                for (auto &j : out_record_data) {
                    ...
                    for (const auto &jj : j) {
                        auto a_value = std::static_pointer_cast<arrow::Int64Scalar>(
                                jj->scalar());
                ...
                auto out_batch = arrow::RecordBatch::Make(out_schema,
                                                          out_arrays[0]->length(),
                                                          out_arrays);
                std::shared_ptr<Block> out_block = std::make_shared<Block>(
                        Block(rand(), out_batch, BLOCK_SIZE));
