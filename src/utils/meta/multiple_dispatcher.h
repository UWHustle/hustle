...
template <typename PredicateTL, typename PrevTL, typename RestCtxTL,
          typename CurrentCtx, typename Enable = void>
struct MultipleDispatcher_1;

template <typename PredicateTL, typename PrevTL, typename RestCtxTL,
          typename VT, typename VL, typename TL, typename ResTL,
          typename... FutureArgTypes>
struct MultipleDispatcher_1<
...
struct MultipleDispatcher_1<
  ...
  static inline auto InvokeOn(const FutureArgTypes &... rest_values,
    ...
    return MultipleDispatcher_1<
        PredicateTL, CurrentTL, typename RestCtxTL::tail,
        typename RestCtxTL::head>::template InvokeOn<Transformer, OnSuccess,
                                                     OnFailure>(
        rest_values..., success, failure);
  }
};
...
struct MultipleDispatcher_1<
  ...
  static inline auto InvokeOn(const OnSuccess &success,
    ...
    return MultipleDispatcher_2<PredicateTL, ResultTL>::template InvokeOn<
        Transformer, OnSuccess, OnFailure>(success, failure);
  }
};

template <typename PredicateTL, typename PrevTL, typename RestCtxTL,
          typename VT, typename VL, typename TL, typename FutureArgTL,
          typename ResTL>
struct MultipleDispatcher_1<
    PredicateTL, PrevTL, RestCtxTL, TypeList<VT, VL, TL, FutureArgTL, ResTL>,
    std::enable_if_t<VL::length != 0 && RestCtxTL::length == 0>> {
  template <typename Transformer, typename OnSuccess, typename OnFailure>
  static inline auto InvokeOn(const VT &value, const OnSuccess &success,
                              const OnFailure &failure) {
    if (value == VL::head::value) {
      using ResultTL = typename PrevTL::template push_back<
          typename TL::head>::template append<ResTL>;
      return MultipleDispatcher_2<PredicateTL, ResultTL>::template InvokeOn<
          Transformer, OnSuccess, OnFailure>(success, failure);
    } else {
      return MultipleDispatcher_1<
          PredicateTL, PrevTL, RestCtxTL,
          TypeList<VT, typename VL::tail, typename TL::tail, FutureArgTL,
                   ResTL>>::template InvokeOn<Transformer, OnSuccess,
                                              OnFailure>(value, success,
                                                         failure);
    }
  }
};
...
struct MultipleDispatcher_1<
  ...
  static inline auto InvokeOn(const VT &value,
                              ...
                              const OnFailure &failure) {
    if (value == VL::head::value) {
      using CurrentTL = typename PrevTL::template push_back<
          typename TL::head>::template append<ResTL>;
      return MultipleDispatcher_1<
          PredicateTL, CurrentTL, typename RestCtxTL::tail,
          typename RestCtxTL::head>::template InvokeOn<Transformer, OnSuccess,
      ...
      return MultipleDispatcher_1<
          ...
          template InvokeOn<Transformer, OnSuccess, OnFailure>(
              value, rest_values..., success, failure);
    }
  }
};
...
struct MultipleDispatcher_0<PredicateTL, Transformer, EntryTL, CtxTL,
                            TypeList<ArgTypes...>,
                            std::enable_if_t<EntryTL::length == 0>> {
  using dispatcher =
      MultipleDispatcher_1<PredicateTL, TypeList<>, typename CtxTL::tail,
                           typename CtxTL::head>;

  template <typename Functor>
  static inline auto InvokeOn(const ArgTypes &... args,
                              const Functor &functor) {
    const auto failure = [](auto emptylist) -> decltype(functor(TypeList<>())) {
      LOG(FATAL) << "Unexpected value for SequenceDispather::InvokeOn";
    };

    return dispatcher::template InvokeOn<Transformer, Functor,
                                         decltype(failure)>(args..., functor,
                                                            failure);
  }

  static inline bool IsInvokable(const ArgTypes &... args) {
    const auto success = [](auto typelist) -> bool { return true; };
    const auto failure = [](auto emptylist) -> bool { return false; };

    return dispatcher::template InvokeOn<IdentityTransformer, decltype(success),
                                         decltype(failure)>(args..., success,
                                                            failure);
  }
};
...
struct MultipleDispatcher_0<
    ...
    : MultipleDispatcher_0<
          ...
          typename CtxTL::template push_front<
              TypeList<typename EntryTL::head::template at<0>,
                       typename EntryTL::head::template at<1>,
                       typename EntryTL::head::template at<2>, FutureArgTL,
...
struct PredicateDelegatorShift {
  ...
  using type = PredicateDelegator<typename PD::predicate, PD::kStart + offset,
                                  PD::kLength>;
};
...
struct ResultLengthForEntry {
  using type = std::integral_constant<
      std::size_t, Entry::template at<3>::length +
...
struct MultipleDispatcher
    : MultipleDispatcher_0<PredicateTL, Transformer, EntryTL, TypeList<>,
                           TypeList<>> {
  using entries = EntryTL;
  using predicates = PredicateTL;

  static constexpr std::size_t kOutputSize =
      entries::template map<ResultLengthForEntry>::template foldl<
          Add, std::integral_constant<std::size_t, 0>>::value;
  ...
  using set_next = MultipleDispatcher<
      ...
      typename predicates::template append<
          typename NextDispatcher::predicates ::template cartesian_product<
  ...
  using set_prev = MultipleDispatcher<
      ...
      typename PrevDispatcher::predicates::template append<
          typename predicates ::template cartesian_product<
              TypeList<std::integral_constant<std::size_t,
                                              PrevDispatcher::kOutputSize>>>::
              template map<
                  PredicateDelegatorShift>>>;  // NOLINT[build/include_what_you_use]
...
struct MultipleDispatcherOverload {
  ...
  static_assert(VL::length > 0, "Sequence cannot be empty.");
};
...
template <typename S, typename... Ts>
struct MultipleDispatcher
    : internal::MultipleDispatcherOverload<
