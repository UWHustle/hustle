#ifndef HUSTLE_XORFILTER_H
#define HUSTLE_XORFILTER_H
...
bool xor8_buffered_populate(const uint64_t *keys, uint32_t size,
  ...
  while (stack_size > 0) {
    ...
    if (ki.index < blockLength) {
      val ^= fingerprints1[xor8_get_h1(ki.hash, filter)] ^
             fingerprints2[xor8_get_h2(ki.hash, filter)];
    } else if (ki.index < 2 * blockLength) {
      val ^= fingerprints0[xor8_get_h0(ki.hash, filter)] ^
             fingerprints2[xor8_get_h2(ki.hash, filter)];
    } else {
      val ^= fingerprints0[xor8_get_h0(ki.hash, filter)] ^
             fingerprints1[xor8_get_h1(ki.hash, filter)];
    }
...
bool xor16_buffered_populate(const uint64_t *keys, uint32_t size,
  ...
  while (stack_size > 0) {
    ...
    uint64_t val = xor_fingerprint(ki.hash);
    if (ki.index < blockLength) {
      val ^= fingerprints1[xor16_get_h1(ki.hash, filter)] ^
             fingerprints2[xor16_get_h2(ki.hash, filter)];
    } else if (ki.index < 2 * blockLength) {
      val ^= fingerprints0[xor16_get_h0(ki.hash, filter)] ^
             fingerprints2[xor16_get_h2(ki.hash, filter)];
    } else {
      val ^= fingerprints0[xor16_get_h0(ki.hash, filter)] ^
             fingerprints1[xor16_get_h1(ki.hash, filter)];
    }
    filter->fingerprints[ki.index] = val;
  }
  xor_free_buffer(&buffer0);
