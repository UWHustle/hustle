...
class BloomFilter {
 public:
   ...
   * otherwise.
   */
  static inline bool compare(const std::shared_ptr<BloomFilter>& lhs,
                             const std::shared_ptr<BloomFilter>& rhs) {
    return lhs->get_hit_rate() < rhs->get_hit_rate();
  }

  static inline bool compare2(const std::shared_ptr<BloomFilter>& lhs,
                              const std::shared_ptr<BloomFilter>& rhs) {
