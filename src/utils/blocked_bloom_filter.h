...
#ifndef HUSTLE_BLOCKEDBLOOMFILTER_H
#define HUSTLE_BLOCKEDBLOOMFILTER_H
...
class BlockedBloomFilter {
 public:
   ...
   */
  explicit BlockedBloomFilter(int num_vals) {
    eps_ = 1e-2;
   ...
   * @return true if the probe was successful (i.e. val is likely in the
  ...
  inline double get_hit_rate_q(int q) { return hist_->get_quantile(q); }
   ...
   * otherwise.
   */
  static inline bool compare(const std::shared_ptr<BlockedBloomFilter>& lhs,
                             const std::shared_ptr<BlockedBloomFilter>& rhs) {
    return lhs->get_hit_rate() < rhs->get_hit_rate();
  }

  static inline bool compare2(const std::shared_ptr<BlockedBloomFilter>& lhs,
                              const std::shared_ptr<BlockedBloomFilter>& rhs) {
   ...
   */
  inline void set_fact_fk_name(std::string fk_name) {
...
#endif  // HUSTLE_BLOCKEDBLOOMFILTER_H
