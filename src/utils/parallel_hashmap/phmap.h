...
inline size_t H1(size_t hash, const ctrl_t*) { return (hash >> 7); }
  ...
  insert_return_type insert(node_type&& node, size_t hash) {
    ...
    auto res = PolicyTraits::apply(
        InsertSlotWithHash<false>{
            *this, std::move(*CommonAccess::GetSlot(node)), hash},
        elem);
    if (res.second) {
      CommonAccess::Reset(&node);
      return {res.first, true, node_type()};
 ...
 private:
  ...
  struct EmplaceDecomposable {
    ...
    std::pair<iterator, bool> operator()(const K& key, Args&&... args) const {
      return s.emplace_decomposable(
          key, typename raw_hash_set::HashElement{s.hash_ref()}(key),
          std::forward<Args>(args)...);
    }
    raw_hash_set& s;
  };
...
class parallel_hash_set {
 ...
 public:
  ...
  parallel_hash_set(size_t bucket_count, const hasher& hash_param,
                    const allocator_type& alloc)
  ...
  parallel_hash_set(InputIter first, InputIter last, size_t bucket_count,
      ...
      : parallel_hash_set(first, last, bucket_count, hash_param, key_equal(),
                          alloc) {}

  template <class InputIter>
  parallel_hash_set(InputIter first, InputIter last, size_t bucket_count,
                    const allocator_type& alloc)
      : parallel_hash_set(first, last, bucket_count, hasher(), key_equal(),
                          alloc) {}
  ...
  parallel_hash_set(std::initializer_list<T> init, size_t bucket_count = 0,
                    ...
                    const key_equal& eq = key_equal(),
                    const allocator_type& alloc = allocator_type())
      : parallel_hash_set(init.begin(), init.end(), bucket_count, hash_param,
                          eq, alloc) {}
  ...
  PHMAP_ATTRIBUTE_REINITIALIZES void clear() {
    for (auto& inner : sets_) inner.set_.clear();
  }
  ...
  float load_factor() const {
    ...
    return capacity ? static_cast<float>(static_cast<double>(size()) / capacity)
                    : 0;
  }
 ...
 protected:
  template <class K = key_type>
  iterator find(const key_arg<K>& key, size_t hashval,
                typename Lockable::SharedLock& mutexlock) {
    Inner& inner = sets_[subidx(hashval)];
    auto& set = inner.set_;
...
class parallel_hash_map
 ...
 public:
  ...
  bool if_contains(const key_arg<K>& key, F&& f) const {
    ...
    typename Lockable::SharedLock m;
    auto it =
        const_cast<parallel_hash_map*>(this)->find(key, this->hash(key), m);
...
struct FlatHashMapPolicy {
  ...
  static decltype(phmap::container_internal::DecomposePair(
      std::declval<F>(), std::declval<Args>()...))
  apply(F&& f, Args&&... args) {
    return phmap::container_internal::DecomposePair(
        std::forward<F>(f), std::forward<Args>(args)...);
  }
...
class flat_hash_map
    : public phmap::container_internal::raw_hash_map<
          phmap::container_internal::FlatHashMapPolicy<K, V>, Hash, Eq, Alloc> {
