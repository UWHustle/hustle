...
public:
  ...
  const HustleType &getHustleType() const {
    return type_;
  }

  const std::optional<int> &getSize() const {
    return size_;
  }
...
public:
  ...
  const std::optional<int> &getTypeSize() const {
  ...
  std::string toString() const {
    return name_ + " " + type_.toString() + " notNull: " +
           std::to_string(notNull_) + " unique: " + std::to_string(unique_);
  }

  template<class Archive>
  void serialize(Archive &archive) {
    archive(CEREAL_NVP(name_), CEREAL_NVP(type_),
            CEREAL_NVP(notNull_), CEREAL_NVP(unique_));
  }
