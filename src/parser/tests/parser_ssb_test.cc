...
TEST_F(ParserSSBTest, ssb_q1) {
  ...
  std::string query =
      "EXPLAIN QUERY PLAN select sum(lo_extendedprice * lo_discount) as "
      "revenue "
...
TEST_F(ParserSSBTest, ssb_q3) {
  ...
  std::string query =
      "EXPLAIN QUERY PLAN select sum(lo_extendedprice*lo_discount) as revenue\n"
      "from lineorder, ddate\n"
      "where lo_orderdate = d_datekey\n"
...
TEST_F(ParserSSBTest, ssb_q4) {
  ...
  std::string query =
      "EXPLAIN QUERY PLAN select sum(lo_revenue), d_year, p_brand1\n"
...
TEST_F(ParserSSBTest, ssb_q7) {
  ...
  std::string query =
      "EXPLAIN QUERY PLAN select c_nation, s_nation, d_year, sum(lo_revenue) "
      "as revenue\n"
...
TEST_F(ParserSSBTest, ssb_q8) {
  ...
  std::string query =
      "EXPLAIN QUERY PLAN select c_city, s_city, d_year, sum(lo_revenue) as "
      "revenue\n"
      "\tfrom customer, lineorder, supplier, ddate\n"
      "\twhere lo_custkey = c_custkey\n"
      "\t\tand lo_suppkey = s_suppkey\n"
...
TEST_F(ParserSSBTest, ssb_q11) {
  ...
  std::string query =
      "EXPLAIN QUERY PLAN select d_year, c_nation, "
...
TEST_F(ParserSSBTest, ssb_q12) {
  ...
  std::string query =
      "EXPLAIN QUERY PLAN select d_year, s_nation, p_category, "
      "sum(lo_revenue-lo_supplycost) as profit1\n"
      "\tfrom ddate, customer, supplier, part, lineorder\n"
      "\twhere lo_partkey = p_partkey\n"
      "\t\tand lo_suppkey = s_suppkey\n"
...
TEST_F(ParserSSBTest, ssb_q13) {
  ...
  std::string query =
      "EXPLAIN QUERY PLAN select d_year, s_city, p_brand1, "
      "sum(lo_revenue-lo_supplycost) as profit1\n"
      "\tfrom ddate, customer, supplier, part, lineorder\n"
      "\twhere lo_partkey = p_partkey\n"
      "\t\tand lo_suppkey = s_suppkey\n"
